...
void init() {
  ...
  initTbh(&flyTbh, 0, .07, .01, .007, 127, true);  //Initialize TBH with coefficients								You will change these numbers to tune the loop
...
task auton() {																																			//AUTONOMOUS
  ...
  //setPidDoRun(&rPid, true);
 	...
 	//if(SensorValue[red]) {
 		...
 		//	movePid(&lPid, 915); //drive straight
  	//	movePid(&rPid, 890);
...
task userOp() {
  ...
  while (true) {
    ...
    if (FIRE_BTN) motor[intake] = motor[lift] = 127;
   	  ...
   	  int feedInSpeed = (!isTbhInThresh(&flyTbh, velThresh) && (SensorValue[leftLim] || SensorValue[rightLim])) ? 0 : 127;
