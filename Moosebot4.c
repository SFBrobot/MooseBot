...
#pragma config(Motor,  port5,           rFly,          tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port6,           lFly,          tmotorVex393HighSpeed_MC29, openLoop, reversed, encoderPort, dgtl1)
...
#define INTAKE_FEEDIN_BTN (vexRT[Btn6U] || vexRT[Btn5UXmtr2])
#define INTAKE_FEEDOUT_BTN (vexRT[Btn6D] || vexRT[Btn5DXmtr2])

#define LIFT_RAISE_BTN (vexRT[Btn6U] || vexRT[Btn6UXmtr2])
#define LIFT_LOWER_BTN (vexRT[Btn6D] || vexRT[Btn6DXmtr2])
...
float flyPwr[4] = {0, 2500, 2800, 3100};
...
task lcd() {
	...
	while (true) {
		...
		if (nLCDButtons & kButtonCenter) {
			...
			displayLCDCenteredString(0, "MB-4800 BUCKETER");
			displayLCDCenteredString(1, "PREP FOR BUCKETS");
		}
		else if (rkAutonMode) {
			bLCDBacklight = true;
			...
			if (pwrBtnsDown) {
				...
				if (pwrBtnLatch.out || (time - pwrBtnTs >= (pwrBtnsRepeating ? pwrBtnsRepeatInterval : pwrBtnsDelayInterval))) {
					...
					pwrBtns = (PWR_BTN_UP ^ PWR_BTN_DOWN) ? (PWR_BTN_UP ? flyPwrIncrement : -flyPwrIncrement) : 0;
			...
			if (time - dispPwrTs <= dispPwrTimeout && flyDir) {
				...
				sprintf(str, "% 07.2f  % 07.2f",
					fmaxf(-9999.99, fminf(9999.99, flyDispFlt.out)),
					fmaxf(-9999.99, fminf(9999.99, flyTbh.err)));
				displayLCDString(0, 0, str);

				sprintf(str, "% 07.2f  % 07.2f",
					fmaxf(-9999.99, fminf(9999.99, fly2Flt.out)),
					fmaxf(-9999.99, fminf(9999.99, flyTbh.out)));
				displayLCDString(1, 0, str);
...
void updateCtl(float dt) {
	updateDiff(&flyDiff, -.5 *SensorValue[flyEnc], dt);
...
#define driveR driveB
	...
	while (true) {
		...
		//motor[intake] =
		//	(INTAKE_FEEDIN_BTN ^ INTAKE_FEEDOUT_BTN) ? (INTAKE_FEEDIN_BTN ? 127 : -127) : 0;

		//motor[lift] =
		//	(LIFT_RAISE_BTN ^ LIFT_LOWER_BTN) ? (LIFT_RAISE_BTN ? 127 : -127) : 0;
