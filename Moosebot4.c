...
#pragma config(Motor,  port4,           mlWheel,       tmotorVex393TurboSpeed_MC29, openLoop)
...
task userOp() {
  ...
  while (true) {
    ...
    motor[slWheel] = arcadeLeft(sSide, sMid, flipLatch.out);
    motor[mlWheel] = arcadeLeft(mSide, mMid, flipLatch.out);

    motor[srWheel] = arcadeRight(sSide, sMid, flipLatch.out);
